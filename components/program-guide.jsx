...
export default function ProgramGuide() {
  return <div className="program-guide">
    ...
    <div id="choose"><PageBlock title="오늘 필요한 문제부터 고르세요.">
      <p>전체 흐름을 점검할 때와 특정 개념을 다시 볼 때, 연습 범위를 다르게 선택해 보세요.</p>
      <div className="guide-visual-row">
        <div className="guide-options">
          <article><GuideIcon kind="paper" /><div><h3>한 회차를 이어서 풀고 싶다면</h3><p>시험지를 선택하세요. 여러 단원을 함께 풀고, 결과에서 다시 확인할 문항을 구분합니다.</p></div></article>
    ...
    <div id="retry"><PageBlock title="해설 전에, 스스로 풀 기회를 한 번 더." tint>
      <div className="guide-visual-row"><div><p>채점 후 바로 해설을 읽기보다, 어디까지 혼자 풀 수 있는지 확인해 보세요.</p>
        <ol className="guide-retry-steps">
          <li><h3>막힌 지점 찾기</h3><p>조건 해석, 식 세우기, 계산 중 어디에서 멈췄는지 짚습니다.</p></li>
    ...
    <div id="handwriting"><PageBlock title="직접 쓴 풀이가, 다음 복습의 단서가 됩니다.">
      <div className="guide-visual-row"><div><p>태블릿과 펜으로 문제 위에 풀어보세요. 복습할 때 처음 세운 식과 계산을 고친 흔적을 되짚을 수 있습니다.</p>
        <div className="guide-handwriting-flow" aria-label="손풀이 제출 순서"><span>직접 쓰기</span><span aria-hidden="true">→</span><span>읽은 답 확인</span><span aria-hidden="true">→</span><span>수정·제출</span></div>
        <h3>제출 전에 답을 확인하세요.</h3><p>앱이 읽어낸 답이 의도한 값과 같은지 확인하고 수정합니다. 채점은 제출 답을 정답과 비교하며, 풀이의 모든 단계를 판단하지는 않습니다.</p>
    ...
    <div id="review"><PageBlock title="다음 공부에서 확인할 것을 골라보세요." tint>
      <p>틀린 문항, 오래 걸린 문항, 다른 학생의 접근을 살펴보고 다음 연습의 초점을 정합니다.</p>
      ...
      <div className="guide-visual-row guide-community"><div><h3>다음에는 무엇을 다르게 해볼까요?</h3><p>예를 들어 ‘조건을 식 옆에 표시하기’, ‘계산 뒤 부호 확인하기’처럼 시도할 행동 하나를 정해 보세요.</p><p>다른 풀이를 읽을 때도 내 접근과 달라지는 지점을 찾아보세요. 위 행동은 스스로 적용할 학습 예시이며 앱이 실수 원인을 자동으로 진단하는 것은 아닙니다.</p>
    ...
    <PageBlock title="내 공부에 맞는 방식으로 시작해 보세요."><p>문제 연습은 로그인 없이 시작할 수 있습니다. 계정 저장과 손풀이 제출에는 로그인이 필요합니다. 공개 베타의 문항 검수 안내와 손풀이 하루 이용 한도를 확인해 주세요.</p><div className="actions"><a className="btn" href="https://111sm.app/mathematics">문제 골라 연습하기</a><Link className="btn secondary" href="/trial">기기·계정 준비 안내</Link></div></PageBlock>
  </div>;
}
