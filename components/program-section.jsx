...
export default function ProgramSection() {
  return <section className="section tint" id="program"><div className="wrap"><div className="section-head"><h2>내 공부 방식에 맞춰 쓰는<br />세 가지 도구.</h2><p>연습 범위를 고르고, 손으로 풀고, 다른 접근을 살펴보세요.</p></div><div className="study-tools">{tools.map(tool => <article key={tool.label}><p className="label">{tool.label}</p><h3>{tool.title}</h3><p>{tool.text}</p></article>)}</div><figure className="video-demo"><div className="demo-top"><span>{"111sm · 손풀이 기능 미리보기"}</span><span>{"27초 미리보기"}</span></div><video autoPlay muted loop controls playsInline preload="metadata" poster="/media/111sm-handwriting-direct-desktop-20260908.jpg" aria-label="손풀이의 답을 읽고 확인하여 제출하는 실제 앱 시연"><source src="/media/111sm-handwriting-direct-desktop-20260908.mp4" type="video/mp4" /></video><figcaption>{"쓰고 → 읽은 답을 확인하고 → 제출합니다."}<br /><span>{"손풀이 기능은 로그인 후 이용 · 시연의 대기 구간은 편집되었습니다."}</span></figcaption></figure></div></section>;
}
